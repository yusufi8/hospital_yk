...
class HospitalAppointment(models.Model):
    ...
    appointment_date = fields.Datetime(
        string="Appointment Date", required=True, tracking=True, default=fields.Datetime.now
    )
    ...
    employee_ids = fields.Selection(
        [('doctor', 'Doctor'), ('nurse', 'Nurse'),
         ('receptionist', 'Receptionist'), ('admin', 'Admin Staff')],
    ...
    def action_done(self):
        for record in self:
            if record.state != 'ongoing':
                raise UserError("Only ongoing appointments can be marked as done.")
            record.state = 'done'
    ...
    def action_share_whatsapp(self):
        for record in self:
            if not record.patient_id.mobile:
                raise UserError("Patient does not have a mobile number.")
            message = f"Hello {record.patient_id.name}, your appointment is scheduled on {record.appointment_date}."
            whatsapp_url = f"https://api.whatsapp.com/send?phone={record.patient_id.mobile}&text={message}"
            record.message_post(body=f"Share via WhatsApp: <a href='{whatsapp_url}' target='_blank'>Click here</a>")
            return {
    ...
    def duplicate_records(self):
        for record in self:
            record.copy()
            record.message_post(body="Appointment record duplicated.")
        print(self)

    def copy(self,defaults=None):
        print(self,defaults)
        rtn = super(HospitalAppointment,self).copy(defaults)
        print(rtn)
        return rtn
