...
class HospitalPatient(models.Model):
    ...
    guardian = fields.Selection(
        ...
        string="Guardian Relation", tracking=True
    )
    guardian_name = fields.Char(string="Guardian Name", tracking=True)
    guardian_mobile = fields.Char(string="Guardian Mobile", tracking=True)
    medical_history = fields.Text(string="Medical History", tracking=True)
    appointment_date = fields.Datetime(string="Next Appointment Date", tracking=True)
    ...
    prescription_ids = fields.Text(string="Prescription", tracking=True)
    meditation = fields.Text(string="Meditation", tracking=True)
    image = fields.Image(
        string="Image", max_width=1920, max_height=1920, tracking=True
    )
    dosage = fields.Text(string="Dosage", tracking=True)
    instructions = fields.Text(string="Instructions", tracking=True)
    ...
    def custom_method(self):
        print("Clicked custom method")

        data = {'name': 'Test Patient', 'age': 30}
        print(self.env['hospital.patient'].create(data))
        pass
    def write(self, vals):
        print("In write method")
        print(vals)
        rtn = super(HospitalPatient,self).write(vals)
        print(rtn)
        return rtn
